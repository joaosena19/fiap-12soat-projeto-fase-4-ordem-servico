...
public class FinalizarExecucaoUseCase
{
    public async Task ExecutarAsync(Ator ator, Guid ordemServicoId, IOrdemServicoGateway gateway, IOperacaoOrdemServicoPresenter presenter, IAppLogger logger, IMetricsService metricsService)
    {
        try
        {
            ...
            RegistrarMetricaTempoExecucao(ordemServico, statusAnterior, statusNovo, metricsService, ator, logger);
    ...
    private void RegistrarMetricaTempoExecucao(OS ordemServico, StatusOrdemServicoEnum statusAnterior, StatusOrdemServicoEnum statusNovo, IMetricsService metricsService, Ator ator, IAppLogger logger)
    {
        try
        {
            // O próprio Value Object HistoricoTemporal garante que as datas não serão nulas.
            var duracaoMs = (ordemServico.Historico.DataFinalizacao!.Value - ordemServico.Historico.DataInicioExecucao!.Value).TotalMilliseconds;
        ...
        catch (Exception ex)
        {
            logger.ComUseCase(this)
                  .ComAtor(ator)
                  .LogError(ex, "Erro ao registrar métrica de tempo de execução. OrdemServicoId: {OrdemServicoId}", ordemServico.Id);
        }
    }
}